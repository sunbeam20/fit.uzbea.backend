...
const decimal = (value) => new library_1.Decimal(value.toString());
// Helper function to generate sequential IDs
function generateSequentialId(prefix, index) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const table of tablesToClear) {
            try {
                ...
                yield prisma[modelName].deleteMany({});
        ...
        for (const permission of permissionsData) {
            try {
                yield prisma.permissions.upsert({
        ...
        for (let i = 0; i < rolesData.length; i++) {
            try {
                const role = yield prisma.roles.upsert({
        ...
        const allPermissions = yield prisma.permissions.findMany();
        const superAdminRole = createdRoles.find((r) => r.name === "Super Admin");
        if (superAdminRole) {
            for (const permission of allPermissions) {
                try {
                    yield prisma.rolePermissions.upsert({
                        where: {
        ...
        for (let i = 0; i < usersData.length; i++) {
            ...
            try {
                const user = yield prisma.users.upsert({
                    ...
                    update: {
                        name: userData.name,
                        password: yield bcryptjs_1.default.hash(userData.password, 10),
                        role_id: role.id,
        ...
        for (let i = 0; i < categoriesData.length; i++) {
            try {
                const category = yield prisma.categories.upsert({
        ...
        for (let i = 0; i < suppliersData.length; i++) {
            try {
                const supplier = yield prisma.suppliers.create({
                    data: Object.assign({ suppId: generateSequentialId("SUP", i) }, suppliersData[i]),
                });
        ...
        for (let i = 0; i < customersData.length; i++) {
            try {
                const customer = yield prisma.customers.create({
                    data: Object.assign({ custId: generateSequentialId("CUST", i) }, customersData[i]),
                });
        ...
        const productsData = [
            // Laptops
            {
                ...
                productType: prisma_1.ProductType.New,
                status: prisma_1.ProductStatus.Active,
            ...
            try {
                const product = yield prisma.products.create({
                    data: {
                        ...
                        supplier_id: supplier === null || supplier === void 0 ? void 0 : supplier.id,
            ...
            if (product.useIndividualSerials) {
                ...
                for (let i = 1; i <= numSerials; i++) {
                    ...
                    serialsData.push({
                        ...
                        status: i <= 2 ? prisma_1.SerialStatus.Available : prisma_1.SerialStatus.Sold, // Mark some as sold
                        warranty: Math.random() > 0.3 ? prisma_1.Warranty.Yes : prisma_1.Warranty.No,
                    });
                }
                try {
        ...
        for (const purchase of createdPurchases) {
            ...
            for (let i = 0; i < numItems; i++) {
                ...
                try {
                    const purchaseItem = yield prisma.purchasesItems.create({
                        data: {
                            quantity,
                            unitPrice,
                    ...
                    if (product.useIndividualSerials) {
                        const serials = yield prisma.productSerials.findMany({
                            where: {
                                product_id: product.id,
                                status: prisma_1.SerialStatus.Available,
                            },
                            take: quantity,
                        });
                        for (const serial of serials) {
                            try {
                                yield prisma.purchaseItemSerials.create({
                                    data: {
                                ...
                                yield prisma.productSerials.update({
                                    where: { id: serial.id },
                                    data: { status: prisma_1.SerialStatus.Available },
                                });
                            }
                            catch (error) {
        ...
        for (const sale of createdSales) {
            ...
            for (let i = 0; i < numItems; i++) {
                ...
                try {
                    ...
                    const itemTotal = unitPrice.toNumber() * quantity - ((discount === null || discount === void 0 ? void 0 : discount.toNumber()) || 0);
                    saleTotal += itemTotal;
                    // Update product quantity
                    yield prisma.products.update({
                        where: { id: product.id },
                    ...
                    if (product.useIndividualSerials) {
                        ...
                        for (const serial of serials) {
                            try {
                                yield prisma.salesItemSerials.create({
                                    data: {
                                ...
                                yield prisma.productSerials.update({
                                    where: { id: serial.id },
                                    data: { status: prisma_1.SerialStatus.Sold },
                                });
                            }
                            catch (error) {
